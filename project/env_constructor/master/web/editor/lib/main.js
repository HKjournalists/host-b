...
function updateShape(shapeId, property, newValue, skipCommand, previousValue){
    ...
    /*
     *待修改的属性可能是Figure对象中的子对象或变量。
     *如Figure.Style是一个包含若干属性的对象，对应的property是style.lineStyle这种形式，
     *因此要通过遍历props先取到子对象名'style'，
     *再通过Figure[style]获取到该对象，然后再对style中的具体属性进行操作。
     *对于简单的属性，不需要用对象进行包装，如Figure.url，此时不需要进行遍历，
     *直接通过props[props.length -1]即可取到该属性的当前值。
     */
    ...
    //wb mod如果修改name属性，则需要同时修改图形中的Text对象显示的值
    if (propName === 'name') {
        objSave.primitives[1].str = newValue;
    }
...
//    Log.info("onMouseDown at (" + x + "," + y + ")");
    ...
    switch(state){
        case STATE_TEXT_EDITING:
            ...
            if (currentTextEditor && currentTextEditor.mouseClickedInside(ev)) {
                break;
            } else {
    ...
    if(cId != -1){
        ...
        if (cId != -1) {
            ...
            if (fId != -1) {     
                ...
                //wb mod create config panel
                state = STATE_TEXT_EDITING;
                setUpConfigPopup(figure, coords);
                redraw = true;
...
function connectorPickFirst(x, y, ev){
    ...
    if(fCpOverId != -1){ //Are we over a ConnectionPoint from a Figure?
        ...
        //将figure上的connectionPoint与connector上的connectionPoint建立对应关系
...
//    Log.group("main.js->getCanvasXY()");
    ...
    if(ev.touches){ //iPad 
        ...
        //console.log("getCancasXY: (" + ev.pageX + "," + ev.pageY + ")");
    }
...
function documentOnMouseMove(evt){
    ...
    switch(state){
        case STATE_FIGURE_CREATE:
            //console.log("mouse moving: (" + evt.pageX + "," + evt.pageY + ")");
...
//wb mod : 整理数组，去掉oldArr中包含'N/A'的元素（头节点除外）
function trimArr(oldArr){
    for(var index in oldArr){
        if(index === 0){
            continue;
        }
        var obj = oldArr[index];
        var flag = 0;
        for(var attr in obj){
...
/**wb mod : 生成json格式的拓扑定义字符串
{
    'server' : [{
        ...
        'staticRoute' : [
            {'destIp' : '', 'mask' : '', 'nextIp' : '', 'iface' : ''}
        ],
        'adminInfo' : {
            'serverName' : '', 'ip' : '', 'username' : '', 'password' : ''
        }
    }],
    'switch' : [{
        'l2Interface' : [
        ...
        'l3Interface' : [
            {'ifName' : '', 'vlanId' : '' , 'ip' : '' , 'mask' : ''}
        ],
        'staticRoute' : [
            {'destIp' : '', 'nextIp' : '', 'mask' : ''}
        ],
        'adminInfo' : {
            'switchName' : '', 'ip' : '', 'username' : '', 'password' : ''
        }
    }]
}
 */
...
function generateJson(){
    //console.log(STACK.figures);
    var data = {
        Server : [],
        Switch : []
    }
    for(i in STACK.figures){
        //console.log(STACK.figures[i].props);
        var dev = STACK.figures[i];
        /*
        if(!dev.props.check()){
            return false;
        }
        */
        ...
        if(dev.devType === 'Server'){
            temp = {
                svInterface : dev.props.ifs,
                svStaticRoute : dev.props.staticRoute,
                adminInfo : {
                    devName : dev.props.name, 
                    ip : dev.props.ip, 
                    userName : dev.props.userName,
                    passwd : dev.props.passwd           
                }
            };
            data['Server'].push(temp);
        }
        else if(dev.devType === 'Switch'){
            temp = {
                l2Interface : dev.props.l2ifs,
                l3Interface : dev.props.l3ifs,
                swStaticRoute : dev.props.staticRoute,
                adminInfo : {
                    devName : dev.props.name, 
                    ip : dev.props.ip, 
                    userName : dev.props.userName,
            ...
            data['Switch'].push(temp);
        }
    }
    JDATA = JSON.stringify(data);  //将JSON对象转化为JSON字符串，发给后台
    //alert("JSON Data: \n" + JDATA);
    submitJson();
}
...
function getDevInfo(ip, username, password, devType, queryType, figureId){
    ...
    var data = {
        ...
        username : username,
        password : password,
        dev : devType,      //devType = 'sw' / 'sv'
        qtype : queryType   //devType = 'L2' / 'L2' / 'If' / 'SR'
    };
    //收到服务器respone时的回调函数
    request.onreadystatechange = function(){
        if(request.readyState === 4 && request.status === 200){
 ...
 */
var PropsHandler = {};
PropsHandler['iup'] = function(ip, uname, passwd) {
    if (!/^((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)$/g.test(ip)){
        alert('非法参数：IP Address');
        return false;
    }
...
PropsHandler['port'] = function(attr) {
    if (!/\d+/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['port'] + '应是整数');
        return false;
    }
    return true
}
PropsHandler['vlanId'] = function(attr) {
    if (!/^\d+$/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['vlanId'] + '应是整数');
        return false;
    }
    return true
}
PropsHandler['ifName'] = function(attr) {
    console.log(attr)
    if (attr.trim() === ''){
        alert('参数为空：' + BuilderProperty.SW_NAMES['ifName']);
        return false;
    }
    return true
}
PropsHandler['ip'] = function(attr) {
    if (!/^((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)$/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['ip']);
        return false;
    }
    return true
}
PropsHandler['mask'] = function(attr) {
    if (!/^([1-2]\d?)|(3[0-2])$/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['mask']);
        return false;
    }
    return true
}
PropsHandler['destIp'] = function(attr) {
    if (!/^((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)$/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['destIp']);
        return false;
    }
    return true
}
PropsHandler['nextIp'] = function(attr) {
    if (!/^((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)$/g.test(attr)){
        alert('非法参数：' + BuilderProperty.SW_NAMES['nextIp']);
        return false;
    }
    return true
}
PropsHandler['iface'] = function(attr) {
    if (attr.trim() === ''){
        alert('参数为空：' + BuilderProperty.SW_NAMES['iface']);
        return false;
    }
    return true
}
PropsHandler['vtype'] = function(attr) {
...
PropsHandler['vmembers'] = function(attr) {
